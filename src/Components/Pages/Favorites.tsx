...
import { Box, Button, Card, Container, Grid, IconButton, Stack, Typography, Pagination } from '@mui/material';
...
const MyFavorites = () => {
  ...
  const { favoriteMeals } = useSelector((state: any) => state.meals);
  ...
  const currentMeals = favoriteMeals?.slice(startIndex, startIndex + itemsPerPage);
      ...
      {favoriteMeals?.length > 0 ? (
        <Box display="flex" justifyContent="center" alignItems="center">
          <Container maxWidth="md">
            <Typography variant="h5" textAlign={"center"} fontWeight={"bold"} mt={2}>My Favorite Meals</Typography>
            <Box display={"flex"} justifyContent={"flex-end"}>
              <Button variant='text' sx={{ marginLeft: "auto" }} onClick={() => navigate(-1)}>Back</Button>
            </Box>
            <Grid container spacing={3} mt={1} mb={20} paddingX={5}>
            ...
            <Box display="flex" justifyContent="center" mt={3}>
              <Pagination
                count={Math.ceil(favoriteMeals.length / itemsPerPage)}
                page={currentPage}
